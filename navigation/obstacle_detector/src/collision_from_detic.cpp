...
class CollisionFromDeticServiceServer
{
private:
    ros::NodeHandle nh_;
...
public:
    CollisionFromDeticServiceServer() : tf_listener_(tf_buffer_)
    {
        detic_client_ = nh_.serviceClient<detic_ros::DeticDetection3DService>("/detic_detection_3d_service");
        map_sub_ = nh_.subscribe("/map", 1, &CollisionFromDeticServiceServer::mapCallback, this);
        risk_sub_ = nh_.subscribe("/navigation/obs_detector/collision_risk", 1, &CollisionFromDeticServiceServer::riskCallback, this);
    ...
    void riskCallback(const std_msgs::Bool::ConstPtr& msg)
    {
        if (!latest_map_)
        {
            ROS_WARN("No map available yet, ignoring collision risk");
            return;
        }

        if (msg->data)
        {
            ROS_INFO("Collision risk detected! Calling Detic service...");
            detic_ros::DeticDetection3DService srv;
            if (detic_client_.call(srv))
            {
                ROS_INFO("Detic service call successful! Detections: %lu", srv.response.detections.detected_object_array.size());
    ...
    void projectCollisionToMap(const detic_ros::DeticDetection3DArray& detections)
    {
        ...
        ROS_INFO("Projection to map started");

        try
        {
            geometry_msgs::TransformStamped transform_stamped = tf_buffer_.lookupTransform(
                "map", "head_rgbd_sensor_rgb_frame", ros::Time(0), ros::Duration(1.0));
            ...
            for (const auto& detection : detections.detected_object_array)
            {
                ...
                for (int i = 0; i < static_cast<int>(cloud.points.size()); ++i)
                {
                    ...
                    // Filter points (range and height)
...
{
    ros::init(argc, argv, "collision_from_detic_service_server");
    CollisionFromDeticServiceServer cfd;
    ros::spin();
    return 0;
}
