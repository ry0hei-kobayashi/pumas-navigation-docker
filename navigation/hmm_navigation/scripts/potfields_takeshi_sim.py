...
def readSensor(data):
     ...
     if (euclD < 0.5) :
        ...
        speed.angular.z=0
         ...
         if( abs(Ftotth) < .35) :#or (np.linalg.norm((Fx,Fy)) < 100):
             speed.linear.x=  min (current_speed.linear.x+0.015, 3.5)#1.9
             print('lin')
             if (current_speed.angular.z >0):
                speed.angular.z = max(current_speed.angular.z-0.1, 0.0)
             if (current_speed.angular.z <0):
                speed.angular.z = min(current_speed.angular.z+0.1, 0.0)
         else:
            if Ftotth > -np.pi*0.25  and Ftotth <0:
                print('Vang-')
                #speed.linear.x  = max(current_speed.linear.x -0.003, 0.04)
                speed.linear.x  = max(current_speed.linear.x -0.03, 0.04)
                speed.angular.z = max(current_speed.angular.z-0.01, -0.6)
            
            if Ftotth < np.pi*0.25  and Ftotth > 0:
                print('Vang+')
                #speed.linear.x  = max(current_speed.linear.x-0.003, 0.04)
                speed.linear.x  = max(current_speed.linear.x-0.03, 0.04)
                speed.angular.z = min(current_speed.angular.z+0.01,0.6)
            
            
            if Ftotth < -np.pi*0.25:
                
                print('Vang---')
                speed.linear.x  = 0#max(current_speed.linear.x-0.025, 0.0)
                speed.angular.z = max(current_speed.angular.z-0.03,-0.85)
            ...
            if Ftotth > np.pi*0.25:
                ...
                print('Vang+++')
                speed.linear.x  = 0#max(current_speed.linear.x-0.025, 0.0)
                speed.angular.z = min(current_speed.angular.z+0.03, 0.85)
         """else:
