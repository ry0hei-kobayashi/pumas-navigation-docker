...
void AugmentedGridMap::addObstacleToMap(const geometry_msgs::PointStamped &added_point)
{
  int cell_x = pointToCell(added_point.point.x, map_metadata.origin.position.x, map_metadata.resolution, static_cast<int>(map_metadata.width));
  int cell_y = pointToCell(added_point.point.y, map_metadata.origin.position.y, map_metadata.resolution, static_cast<int>(map_metadata.height));
 
  if (cell_x < 0 || cell_x >= map_metadata.width || cell_y < 0 || cell_y >= map_metadata.height) {
  ...
  makeObstaclesMarkers(min_x, max_x, min_y, max_y, (min_x + max_x) / 2, (min_y + max_y) / 2);
}
...
bool AugmentedGridMap::clearMapCallback(std_srvs::Empty::Request& request, std_srvs::Empty::Response& response)
{
  ...
  // Publish marker to clear obstacles in RViz
...
void AugmentedGridMap::makeObstaclesMarkers(float min_x, float max_x, float min_y, float max_y, float cen_x, float cen_y)
{
  ...
  try {
    ...
    if (config["furniture"]) {
      for (const auto& it : config["furniture"]) {
        ...
        for (int i = pointToCell(min_x, map_metadata.origin.position.x, map_metadata.resolution, static_cast<int>(map_metadata.width)); i < pointToCell(max_x, map_metadata.origin.position.x, map_metadata.resolution, static_cast<int>(map_metadata.width)); ++i) {
          for (int j = pointToCell(min_y, map_metadata.origin.position.y, map_metadata.resolution, static_cast<int>(map_metadata.height)); j < pointToCell(max_y, map_metadata.origin.position.y, map_metadata.resolution, static_cast<int>(map_metadata.height)); ++j) {
            enhanced_map.data[i + j * map_metadata.width] = 100;
          }
        }
